...
[Tips : Appendix-A 古老的电路构建方式](#Appendix-A)
...
## design 
<u>原理图</u>和<u>PCB数据文件</u>是PCB设计的核心，所有PCB设计软件也为此构造了两个设计模块：”原理图设计“、“PCB Layout”；原理图使用网络描述了元件引脚之间的电气连接，PCB Layout利用原理图的网络产生引脚索引以指导布线。

Tips : PCB设计的重点是电路和电磁兼容性的设计，本质是对模电、数电和电磁理论的运用。~设计软件本身并不重要~。

### steps
#### A - schema
1. 设计电路 : 确认使用的电路、元件及数量 
2. 制作元件 : 制作所使用的元件的原理图
3. 建立网络 ：使用网络工具建立相关元件引脚之间的电气连接

Tips : 原理图中的元件是真实元件的数据抽象，主要表达元件的引脚功能和数量。PCB设计软件中都含有常用元件库，可以直接调用，不足的自己封装。
...
## <span id = "Appendix-A">Appendix-A：古老的电路构建方式 </span>
...
## <span id = "Appendix-B">Appendix-B：PCB 制造工艺 </span>
TBD

## <span id = "Appendix-C">Appendix-C：PCBA </span>
...
## <span id = "Appendix-D">Appendix-D：EMC/EMI/EMS </span>

Electromagnetic compatibility (EMC) 电磁兼容性，是设备或系统在电磁环境中能**抗干扰**运行并对所在环境产生**受限的电磁干扰**的能力，包含EMS (Electromagnetic susceptibility) 电磁耐受性和EMI (Electromagnetic interference) 电磁干扰
...
发射
	...
	脉冲噪声、放射电磁场、静电、雷击、电压变动等


国际上对于电子、电器、工业设备产品的抗扰性测试日渐重视，且趋向整合以IEC国际规格为测试标准
