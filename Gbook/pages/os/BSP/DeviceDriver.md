...
## 源码
内核模块的编译依赖于内核源码，可以把内核源码分为两种：PC宿主机Linux系统下的内核源码和单独下载的内核源码。
...
目标板Linux内核源码可以在https://www.kernel.org/下载，与宿主机内核源码不同的是，它多用于嵌入式工程，且不能直接作为开发环境使用，它需要进一步配置。从这个意义上讲，宿主机上的内核源码更应该称为“内核开发包”（kernel_devel），它不是一个完整的内核。同样的，目标板内核源码版本也要与目标板正在使用的内核一致，通常这个源码应该是移植内核时使用的、已经配置好的那一份。
...
/*
   所包含的头文件在内核源码树中，init_module()和cleanup_module是内核函数，类似C++的构造函数和析构
   函数MODULE_LICENSE(“GPL”);使用此宏(在linux/module.h中)来使自己的代码遵循GPL协议，否则会有“垃圾”信息
   提示，在module.h中还有类似的协议可选，如MODULE_AUTHOR、MODULE_DESCRIPTION、MODULE_ALIAS(“xxx”)，不
   使用协议的另一个可能的后果是不能正确调用内核函数。

   printk()是内核打印函数，不同于C库中的printf()，前者具有等级划分，可设置在不同地点打印消息。
 */
